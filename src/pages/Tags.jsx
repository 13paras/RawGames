...
const Tags = () => {
  ...
  const fetchInitialData = async () => {
    ...
    fetchDataFromApi(`games?tags=${tagId}&ordering=${apiOrderBy}`)
      .then((response) => {
  ...
  const fetchMoreData = async () => {
    ...
    fetchDataFromApi(`games?page=${pageNum}&tags=${tagId}&ordering=${orderBy}`)
      .then((response) => {
        setGamesData({
          ...gamesData,
  ...
  const tagName = gamesData?.results?.[0]?.tags?.find(
    (item) => parseInt(item.id) === parseInt(tagId)
  )?.name;
      ...
      {gamesData && (
        <>
          <div className='mx-6 items-center justify-between lg:flex'>
            <h2 className='my-7 ml-10 text-5xl font-extrabold capitalize'>
              {tagName} Games
            </h2>
